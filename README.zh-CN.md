...
- `http://localhost:8000/admin/` 打开管理端

首次完成有效索引的最小配置：

- **向量化**（详见下文「环境变量」）：默认 **`EMBED_PROVIDER=ollama`** 时需 **`OLLAMA_BASE_URL`**、**`EMBED_MODEL`**；**`EMBED_PROVIDER=openai`** 时需 **`OPENAI_EMBED_BASE_URL`**、**`OPENAI_EMBED_API_KEY`** 及 OpenAI 的 **`EMBED_MODEL`**（如 `text-embedding-3-small`）。嵌入用的 OpenAI 地址/密钥与对话用的 **`OPENAI_*` 相互独立**。
- **私有 HTTPS 仓库（可选）**：`GIT_HTTPS_TOKEN` 或 `GITLAB_ACCESS_TOKEN`
- **Webhook 验签（建议非内网时开启）**：配置各平台 secret
...
parse_functions：Tree-sitter 函数级解析（0 条则 file-level fallback）
  ↓
describe_chunks（可选）：按 `LLM_PROVIDER` 选用 Dify / Azure OpenAI / OpenAI 兼容生成一行描述
  ↓
generate_wiki：静态 Wiki（MkDocs / Starlight / VitePress，站内搜索）→ DATA_DIR/wiki_sites
  ↓
upsert_vector_store：按 `EMBED_PROVIDER`（Ollama 或 OpenAI 兼容）做 embedding → Chroma 写入
  ↓
...
- **向量化**：配置 **`EMBED_PROVIDER`**（`ollama` 或 `openai`）及对应变量（见「环境变量」）；**更换供应商或嵌入维度后需清空 `DATA_DIR/chroma` 并全量重索引**
- **可选 LLM**：设置 **`LLM_PROVIDER`** 为 `dify`、`azure_openai` 或 `openai`（默认 `openai`），并只配置该供应商所需项（见「LLM 供应商」）
...
- **DELETE** `/api/projects/{project_id}`：删除项目对应向量与元数据
- **POST** `/api/projects/{project_id}/reindex`：对项目发起重建入队
- **GET** `/api/projects/{project_id}/vectors`：分页查看向量明细
- **GET** `/api/project/index-status?project_id=xxx`：查看某项目是否已写入向量（`indexed/doc_count`）

### 管理端 / 鉴权 / 运维接口

- **鉴权相关**：`GET /api/auth/status`、`POST /api/auth/login`、`GET /api/auth/me`
- **管理端设置**：`GET /api/admin/settings`
- **存储概览**：`GET /api/admin/storage`
- **LLM 用量统计**：`GET /api/admin/llm-usage`
...
多数项也可在管理端 **「设置」**（`/admin/`）中修改；写入 `DATA_DIR/ui_overrides.json` 的覆盖值优先于 `.env`（以服务端支持的键为准）。
...
**向量化** — 通过 **`EMBED_PROVIDER`** 选择 `ollama`（默认）或 `openai`：

| 变量 | 适用 | 说明 |
|------|------|------|
| `EMBED_PROVIDER` | 始终 | `ollama`（默认）或 `openai`。 |
| `OLLAMA_BASE_URL` | `EMBED_PROVIDER=ollama` | Ollama 基地址（默认 `http://localhost:11434`；Docker 常用 `http://host.docker.internal:11434`）。 |
| `OLLAMA_API_KEY` | 可选 | 经网关访问 Ollama 时的 Bearer 令牌。 |
| `EMBED_MODEL` | 始终 | **Ollama** 下为 Ollama 嵌入模型名；**OpenAI** 下为 embeddings 模型名（如 `text-embedding-3-small`）。**更换供应商/模型/维度后需清空 `DATA_DIR/chroma` 并全量重索引。** |
| `OPENAI_EMBED_BASE_URL` | `EMBED_PROVIDER=openai` | OpenAI 兼容 **嵌入** API 基地址（通常以 `/v1` 结尾）。与对话用的 **`OPENAI_BASE_URL` 独立**。 |
| `OPENAI_EMBED_API_KEY` | `EMBED_PROVIDER=openai` | 仅用于嵌入的 API Key。与 **`OPENAI_API_KEY` 独立**。 |
...
| `EMBED_MAX_CHARS` | 单条文本送入嵌入前的最大字符数，超长截断（默认 `30000`）。 |
| `CONTENT_LANGUAGE` | `zh` 或 `en`；LLM/Wiki 输出语言（默认 `zh`）。 |
| `INDEX_EXCLUDE_PATTERNS` | 索引时跳过的路径 glob，逗号或换行分隔。 |

### LLM 供应商

为 chunk 生成一行描述及 **代码问答** 只使用 **一个** 后端，由 **`LLM_PROVIDER`** 指定：`dify`、`azure_openai` 或 **`openai`**（默认）。只需配置所选供应商对应变量。

| 变量 | 说明 |
|------|------|
| `LLM_PROVIDER` | `dify` \| `azure_openai` \| `openai`（默认 `openai`）。 |
| `DIFY_API_KEY` | Dify Key（`LLM_PROVIDER=dify` 时）。 |
| `DIFY_BASE_URL` | Dify API 地址，默认 `https://api.dify.ai/v1`。 |
| `AZURE_OPENAI_API_KEY` | Azure Key（`LLM_PROVIDER=azure_openai` 时）。 |
...
| `OPENAI_API_KEY` | **对话**用 OpenAI（或兼容）Key（`LLM_PROVIDER=openai` 时）。 |
| `OPENAI_BASE_URL` | 对话/补全接口基地址（默认 `https://api.openai.com/v1`）。 |
| `OPENAI_MODEL` | 对话模型或部署名。 |

若所选供应商未配齐密钥等，索引与检索仍可跑；描述可能缺失或为占位，代码问答无法调模型。

### 高级控制项

| 变量 | 说明 |
|------|------|
| `REPOS_CACHE_MAX_GB` | 本地 Git 镜像目录（`DATA_DIR/repos`）总占用上限（GiB）；超出时按 LRU 删除**其它**项目目录；`0` 不启用 |
| `REPOS_CACHE_MAX_COUNT` | `repos` 下最多保留的子目录个数（含当前任务）；超出时按 LRU 删除其它项目；`0` 不启用 |
...
- **仓库镜像**：`DATA_DIR/repos/<project_id>/...`（多项目时可设 `REPOS_CACHE_MAX_GB` / `REPOS_CACHE_MAX_COUNT` 自动删久未用的其它项目镜像以省磁盘；当前索引任务对应目录不删）
- **向量库**：`DATA_DIR/chroma/`
...
当新增或变更公开 API、队列行为、环境变量、管理端页面时：

- 同一 PR 内同时更新 `README.md` 与 `README.zh-CN.md`。
- 两个文档保持相同章节顺序与接口覆盖范围。
- 至少保留一个可直接运行的 curl 示例，并在变更后可用。
