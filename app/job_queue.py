...
# from __future__ import annotations  # duplicate (ignored)
...
class IndexJobQueue:
    ...
    def _run_loop(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            self._current_job_id = job_id
            started = _utc_now_iso()
            self.store.update_job(job_id, status="running", progress=1, step="starting", message="开始执行", started_at=started)
            ...
            try:
                run_index_pipeline(repo_url=job.repo_url, project_id=job.project_id, progress=reporter)
                finished = _utc_now_iso()
                self.store.update_job(job_id, status="succeeded", progress=100, step="done", message="完成", finished_at=finished)
            except Exception as e:  # noqa: S110
                finished = _utc_now_iso()
    ...
    def _run_loop(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            try:
                run_index_pipeline(repo_url=job.repo_url, project_id=job.project_id, progress=reporter)
                finished = _utc_now_iso()
                # 若 pipeline 自己已上报 done/failed，这里仍以执行无异常为成功
                self.store.update_job(job_id, status="succeeded", progress=100, step="done", message="完成", finished_at=finished)
            except Exception as e:  # noqa: S110
                finished = _utc_now_iso()
                self.store.update_job(job_id, status="failed", step="failed", message=f"失败: {e}", finished_at=finished)
...
class Job:
    ...
    project_id: str
    repo_url: str
    source: str  # webhook / trigger / other
    created_at: float
...
class JobQueue:
    """
    目标：
    - 把“索引/分析”请求变成队列串行执行（避免并发写 Chroma / 同 repo 目录导致失败）
    - 提供任务列表与进度查询
    """
    ...
    def __init__(self) -> None:
        ...
        self._jobs_path: Path = settings.data_path / "jobs.jsonl"
        self._max_keep = int(getattr(settings, "jobs_max_keep", 2000) or 2000) if hasattr(settings, "jobs_max_keep") else 2000
    ...
    def _load_from_disk(self) -> None:
        try:
            if not self._jobs_path.exists():
                return
            # 只加载最后 max_keep 条，避免文件太大
            lines = self._jobs_path.read_text(encoding="utf-8", errors="ignore").splitlines()[-self._max_keep :]
            for ln in lines:
                if not ln.strip():
                    continue
                try:
    ...
    def enqueue(self, *, repo_url: str, project_id: str, source: str) -> Job:
    ...
    def list_jobs(self, limit: int = 50) -> list[dict[str, Any]]:
        ...
        return [j.to_public_dict() for j in jobs[: max(1, min(limit, 200))]]
        ...
        with self._lock:
            ...
            for k, v in fields.items():
                if hasattr(job, k):
                    ...
                    job.meta[k] = v
            # 进度字段做一点规范化
            if isinstance(job.percent, float):
                job.percent = int(job.percent)
    ...
    async def _worker_loop(self) -> None:
        ...
        while True:
            ...
            def reporter(payload: dict[str, Any]) -> None:
                ...
                updates: dict[str, Any] = {"stage": stage or "running"}
                if percent is not None:
                    updates["percent"] = percent
                if message:
                    updates["message"] = str(message)
                # pipeline 结束时会给 status=done/failed/skipped
                if status in {"done", "failed", "skipped"}:
                    updates["status"] = status
                # 其他字段进 meta
                for k, v in payload.items():
                    if k in {"stage", "percent", "status", "message"}:
                        continue
                    updates[k] = v
                self._update(job_id, **updates)

            try:
                # 在线程池里跑同步 pipeline，避免阻塞事件循环
                ...
                final = self.get_job(job_id) or {}
                if final.get("status") == "running":
                    self._update(job_id, status="done", stage="done", percent=100)
            except Exception as e:
                logger.exception("Job %s failed: %s", job_id, e)
                self._update(job_id, status="failed", stage="done", percent=100, message=str(e))
            finally:
...
class JobStore:
    """
    极简持久化任务状态（SQLite）。
    - 目标：任务排队可见、进度可查、服务重启后仍能看到历史记录
    - 注意：这是“单进程/单容器”的实现；若你需要多副本/多 worker，需换成 Redis/Celery/RQ 等分布式队列
    """
    ...
    def reset_incomplete_jobs_on_startup(self) -> int:
        """
        进程重启后，避免把此前“running”卡死的任务永远留在 running。
        策略：把 running -> queued，并标注 message。
        """
        with self._lock:
            rows = self._conn.execute(
                "SELECT job_id FROM index_jobs WHERE status IN ('running')"
            ).fetchall()
            ids = [r["job_id"] for r in rows]
            for job_id in ids:
                self._conn.execute(
...
class IndexJobQueue:
    ...
    def start(self) -> None:
        if self._worker and self._worker.is_alive():
            return
        # 启动前重置 running 任务，避免卡死
        reset = self.store.reset_incomplete_jobs_on_startup()
        if reset:
            logger.warning("Reset %s running jobs to queued on startup.", reset)
        # 把已排队任务重新灌入内存队列（保持 created_at 顺序）
        for job_id in self.store.iter_queued_job_ids():
            self._q.put(job_id)
        self._worker = threading.Thread(target=self._run_loop, name="index-job-worker", daemon=True)
        self._worker.start()
